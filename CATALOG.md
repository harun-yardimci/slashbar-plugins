...
5 plugin pack(s). Install from **Slashbar → Settings → Plugins**.
...
Ports\, sockets and the processes holding them — by **Slashbar** · kinds: Shell ⚠️ *runs shell commands*

[Homepage](<https://github.com/harun-yardimci/slashbar-plugins>) · `dev.slashbar.net-tools`

| Command | Kind | Description |
|---|---|---|
| Who Has This Port\? | Shell | Type a port number to see which process is listening on it\. |
| Common Dev Ports | Shell | Just the ports dev servers usually grab — 3000\, 5173\, 8080 and friends\. |
| Listening Summary | Shell | One tidy line per listening process and port\. |
| Established Connections | Shell | What is actually talking to the network right now\. |
| Check Remote Port | Shell | Type \"example\.com 443\" to see whether that port answers\. |
| My IP Addresses | Shell | Local interface addresses and the public one\. |
